...
export default function Sidebar({ examDates, subjects, selectedDate, onSelectDate, onDeleteDate, onEditDate, isOpen, onToggle }: SidebarProps) {
  return (
    <>
      <button
        className="lg:hidden fixed top-4 left-4 z-40 p-2 rounded-lg bg-white dark:bg-slate-800 shadow-lg border border-slate-200 dark:border-slate-700"
        onClick={onToggle}
      ...
      <aside className={`
        fixed lg:static inset-y-0 left-0 z-30 w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700
      ...
      `}>
        <div className="p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="lg:hidden text-slate-400 hover:text-slate-600" onClick={onToggle}>×</button>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    />
                    <button
                      className="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-600 text-sm ml-2 transition-opacity"
                      onClick={e => { e.stopPropagation(); onDeleteDate(dateInfo.date) }}
                      title="Delete date"
                    >×</button>
                  </div>
                  <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{dateInfo.day}</div>
                  <div className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{count} paper{count === 1 ? '' : 's'}</div>
                </div>
              )
            })}
          </div>
