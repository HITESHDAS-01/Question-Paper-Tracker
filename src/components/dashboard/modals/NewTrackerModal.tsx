      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Unit Test 1, Term II 2026-27" maxLength={100}
          ...
          <div>
            ...
            <input type="text" value={sub} onChange={e => setSub(e.target.value)} placeholder="e.g. Grade VI to X, ABC Public School" maxLength={200}
              className="w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
          ...
          <button
            ...
            onClick={() => {
              ...
              if (!trimmed || trimmed.length > 100) { alert('Tracker name must be 1-100 characters'); return }
              if (sub.trim().length > 200) { alert('Subtitle must be at most 200 characters'); return }
              const g = groups.split(',').map(s => s.trim()).filter(Boolean)
              if (!g.length) { alert('Enter at least one class/group'); return }
              if (g.some(item => item.length > 100)) { alert('Each class/group must be at most 100 characters'); return }
              onSubmit(trimmed, sub.trim(), g, bpMs)
            }}
